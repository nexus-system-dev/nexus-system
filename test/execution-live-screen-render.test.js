...
test("execution live screen renders workspace contract inside visible surface", () => {
  ...
  assert.match(html, /web-app-הרצה → web-product-release/);
  assert.match(html, /התקדמות הבנייה/);
  assert.match(html, /ה־surface מתפתח/);
  assert.match(html, /התפתחות לפי סוג המוצר/);
  assert.match(html, /workflow-evolution/);
  assert.match(html, /workflow-sequence/);
  assert.match(html, /רצף עבודה מקומי/);
  assert.match(html, /browser-backed-local-workspace/);
  assert.match(html, /deferred-to-w4-mbn-010/);
  assert.match(html, /עבודה מקומית/);
  assert.match(html, /browser-backed-shell/);
  assert.match(html, /scope-defined-not-implemented/);
  assert.match(html, /מסלול הרצה/);
  assert.match(html, /web-app-הרצה → web-product-release/);
  assert.match(html, /הכנה לתצוגה ושיתוף/);
  assert.match(html, /deployable-product-web-bundle/);
  assert.match(html, /saas-package → web-deployment/);
  assert.match(html, /product-workspace-preview → product-workspace-preview/);
  assert.match(html, /מוכנות לשיתוף/);
  ...
  assert.match(html, /מסלול שחרור/);
  assert.match(html, /product-web-release-path/);
  assert.match(html, /verify-הרצה-and-promote-service/);
  assert.match(html, /render/);
  assert.match(html, /מצב השחרור/);
