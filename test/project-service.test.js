...
test("project service serialization preserves nexus positioning", () => {
  ...
  assert.equal(typeof result.aiGenerationObservability?.observabilityId, "string");
  ...
  assert.equal(typeof result.promptContractFailureTracker?.trackerId, "string");
  assert.equal(typeof result.promptContractFailureTracker?.failureSummary?.blockingFailureCount, "number");
  assert.equal(typeof result.aiGenerationReviewDashboard?.dashboardId, "string");
  assert.equal(typeof result.aiGenerationReviewDashboard?.summary?.riskSignalCount, "number");
  assert.equal(typeof result.generatedSurfaceProofSchema?.proofId, "string");
  assert.equal(typeof result.generatedSurfaceProofSchema?.summary?.failedCheckCount, "number");
  assert.equal(typeof result.generatedSurfacePerformanceBudgetValidator?.performanceBudgetValidatorId, "string");
  assert.equal(typeof result.generatedSurfacePerformanceBudgetValidator?.summary?.budgetStatus, "string");
  assert.equal(typeof result.generatedBrandConsistencyValidator?.brandConsistencyValidatorId, "string");
  assert.equal(typeof result.generatedBrandConsistencyValidator?.summary?.brandStatus, "string");
  assert.equal(typeof result.renderableDesignProposal?.proposalId, "string");
  assert.equal(typeof result.designProposalValidation?.status, "string");
  assert.equal(typeof result.designProposalReviewState?.status, "string");
  assert.equal(typeof result.generatedAssetProvenanceRecord?.provenanceRecordId, "string");
  assert.equal(typeof result.proposalApplyDecision?.status, "string");
  assert.equal(typeof result.acceptedScreenState?.status, "string");
...
test("project service connects a project to git hosting and stores repo snapshot", async () => {
  ...
  assert.equal(project.sourceControlIntegration.status, "ready");
